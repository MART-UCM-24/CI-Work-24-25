...
env = DifferentialDriveEnv(grid_map=map_1,device=device)
...
for episode in range(episodes):
    ...
    for t in range(max_steps):
        ...
        replay_buffer.add((state.cpu().numpy(), action.cpu().numpy(), next_state.cpu().numpy(), reward.cpu().numpy(), done.cpu().numpy()))
        
        current_rewards[len(current_rewards)-1].append(reward.cpu().numpy())
...
print(f"Time of finish {time.time()}")
