...
class DDPG:
    def __init__(self, state_dim, action_dim, max_action,device):
        self.device = device
        self.actor = Actor(state_dim, action_dim, max_action,device)#.to(device)
        self.actor_target = Actor(state_dim, action_dim, max_action,device)#.to(device)
        self.actor_target.load_state_dict(self.actor.state_dict())
        self.actor_optimizer = optim.Adam(self.actor.parameters(), lr=1e-4)

        self.critic = Critic(state_dim, action_dim,device)#.to(device)
        self.critic_target = Critic(state_dim, action_dim,device)#.to(device)
